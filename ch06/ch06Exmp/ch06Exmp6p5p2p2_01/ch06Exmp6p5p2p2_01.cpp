// ch06Exmp6p5p2p2_01.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
double expression() {
    ...
    switch (t.kind) {  // see which kind og token it is
    case '+':
        return left + expression();  // read and evaluate an expression, then do an add
    case '-':
        return left - expression();  // read and evaluate an expression, then do a subtraction
    default:
